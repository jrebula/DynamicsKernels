classdef ThreeDWalkerSplayFromGroundState < ThreeDWalkerState
  ...
  methods
    ...
    function [this] = ThreeDWalkerSplayFromGroundState(input)
      %%
      ...
      if (isa(input, 'ThreeDWalkerState'))
        ...
        % nominalLimitCycleSplayed.stanceAnkle.pitch = ;
        
        this.stanceAnkle.pitch = input.stanceLeg.pitch;
        this.stanceAnkle.roll = 0;
        
        this.stanceAnkle.pitchDot = input.stanceLeg.pitchDot;
        this.stanceAnkle.rollDot = 0;
      ...
      if (isnumeric(input))
        ...
        %                 else
        this = ThreeDWalkerSplayFromGroundState();
    ...
    function [this] = setStanceAnglesCompatibleWithGround(this)
      %%
      this.setQsUsAndTrigInCurrentFunction();
      
      rollAngleOfSwingAnkle = acos(-(c8*s4*s7) + c4*(s5*(-(c7*c8*s6) - c6*s8) + ...
        c5*(c6*c7*c8 - s6*s8)));
      
      pitchAngleOfSwingAnkle = acos(-(c8*s4*s7) + c4*(s5*(-(c7*c8*s6) - c6*s8) + ...
        c5*(c6*c7*c8 - s6*s8)));
      
      this.stanceAnkle.roll = pitchAngleOfSwingAnkle;
      this.stanceAnkle.pitch = rollAngleOfSwingAnkle;
    end
    
    function [new] = switchLegs(this, walker)
      %% go from walking on old stance foot to walking on old swing foot
      
      new = ThreeDWalkerSplayFromGroundState(walker.findSwitchedWalkerState(this));
      return;
      
      %       error('need to use a walker to switch this state!');
      ...
      new = ThreeDWalkerSplayFromGroundState();
    ...
    function [this] = setFromVector(this, vect)
      %%
      if (length(vect) == 12)
        error('don''t setFromVector with ThreeDWalkerSplayFromGroundState');
    ...
    function [] = setQsUsAndTrigInCurrentFunction(this)
      ...
      assignin(ws, 's6m8p7', sin(x(6) + x(7) - x(8)));
      assignin(ws, 's4m8p5', sin(x(4) + x(5) - x(8)));
