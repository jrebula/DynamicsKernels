...
trigreplaced=Map[Head,alltrig]/.Apply[Rule,trigreps,{1}];
	(* make a list of replacements, starting with q[n_]:>ToString[n], followed by alternative arg replacements if any (whether in vector or matrix form) e.g. {q[n_]:>ToString[n],gamma->"g"} *)
...
(* Use the two lists to form a set of rules, {Sin[q[1]]->"s1",Cos[q[2]]->"c2"} *)
trigSubs=MapThread[Rule[#1,#2]&,{trigfncalls[[reorder]],trigreplacements[[reorder]]}]]


formqureplacements:=Module[{qSubs,uSubs,upSubs,quSubs},qSubs=(q[#1]->ToString[StringForm["q`1`", #]]&)/@qdofs;uSubs=(u[#1]->ToString[StringForm["u`1`", #]]&)/@udofs;upSubs=(Derivative[1][u[#1]]->ToString[StringForm["u`1`dot", #]]&)/@udofs;quSubs=Join[qSubs,uSubs,upSubs]];


pSub = {(x_)^2 -> HoldForm[x*x]}; 


Options[ExportExpression]={qusubs->Automatic,TrigSubs->Automatic,psub->{(x_)^2->HoldForm[x*x]}};
ExportExpression[Mx_?SymmetricQ, name_String,opts___]:=Module[{elementsreplaced,quSubs,trigSubs,pSub},
...
ToString[StringForm["`1`(`2`,`3`) = `4`; ",name,i,j,If[i>j,StringForm["`1`(`2`,`3`)",name,j,i],CForm[Mx[[i,j]]/.trigSubs/.quSubs/.pSub]/.lowercasefunctions]]],"\""->""],
{i,1,Dimensions[Mx][[1]]},{j,1,Dimensions[Mx][[2]]}];
(* output each row as a separate formatted output, with line breaks in between *)
StringJoin[Map[StringInsert[fmtString[#,opts],"\n",-1]&,Map[StringJoin,elementsreplaced]]]];
ExportExpression[Mx_List?MatrixQ, name_String,opts___]:=Module[{elementsreplaced,quSubs,trigSubs,pSub},
quSubs=qusubs/.{opts}/.Options[ExportExpression]/.Automatic->formqureplacements;
trigSubs=TrigSubs/.{opts}/.Options[ExportExpression]/.None->{}/.Automatic->formtrigreplacements[Mx,opts];
pSub=psub/.{opts}/.Options[ExportExpression];
...
StringReplace[ (* strip quotes *)
(* expression of the form name(i,j) = rhs *)
ToString[StringForm["`1`(`2`,`3`) = `4`; ",name,i,j,CForm[Mx[[i,j]]/.trigSubs/.quSubs/.pSub]/.lowercasefunctions]],"\""->""],
{i,1,Dimensions[Mx][[1]]},{j,1,Dimensions[Mx][[2]]}];
(* output each row as a separate formatted output, with line breaks in between *)
StringJoin[Map[StringInsert[fmtString[#,opts],"\n",-1]&,Map[StringJoin,elementsreplaced]]]];
ExportExpression[rhs_List?VectorQ, name_String,opts___]:=Module[{elementsreplaced,quSubs,trigSubs,pSub},
quSubs=qusubs/.{opts}/.Options[ExportExpression]/.Automatic->formqureplacements;
...
StringReplace[ (* strip quotes *)
(* expression of the form name(i,j) = rhs *)
ToString[StringForm["`1`(`2`) = `3`; ",name,i,CForm[rhs[[i]]/.trigSubs/.quSubs/.pSub]/.lowercasefunctions]],"\""->""],
{i,1,Length[rhs]}];
(* output each row as a separate formatted output, with line breaks in between *)
StringJoin[Map[StringInsert[fmtString[#,opts],"\n",-1]&,elementsreplaced]]];
ExportExpression[scalar_ /;Nor[VectorQ[scalar],MatrixQ[scalar] ],name_String ,opts___]:=Module[{quSubs,trigSubs,pSub},
quSubs=qusubs/.{opts}/.Options[ExportExpression]/.Automatic->formqureplacements;
...
Options[ExportCode]={OutputFile->None,Forces->None,Expressions->None};
...
forces=Forces/.{opts}/.Options[ExportCode]/.None->{};
...
expressions=Replace[Expressions/.{opts}/.Options[ExportCode]/.None->{}, x:{_,_String}->{x}];
